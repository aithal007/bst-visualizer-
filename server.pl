...
%  Handles insert operation via HTTP.
handle_insert(Request) :-
    http_read_json_dict(Request, Data),
    Value = Data.value,
    Tree = Data.tree,
    
    % Convert JSON tree to Prolog term
...
%  Handles delete operation via HTTP.
handle_delete(Request) :-
    http_read_json_dict(Request, Data),
    Value = Data.value,
    Tree = Data.tree,
...
%  Handles lookup operation via HTTP.
handle_lookup(Request) :-
    http_read_json_dict(Request, Data),
    Value = Data.value,
    Tree = Data.tree,
...
%  Handles traversal operations via HTTP.
handle_traversal(Request) :-
    http_read_json_dict(Request, Data),
    Type = Data.type,
    Tree = Data.tree,
...
%  Handles building a tree from a list of values.
handle_build(Request) :-
    http_read_json_dict(Request, Data),
    Values = Data.values,
...
%  Handles retrieving tree statistics.
handle_stats(Request) :-
    http_read_json_dict(Request, Data),
    Tree = Data.tree,
...
%  Converts a JSON representation to a Prolog tree term.
...
%  Converts a Prolog tree term to JSON representation.
...
%% demo_tree(-Tree)
%  Creates a demo tree for testing.
demo_tree(Tree) :-
...
%% If you want to use just the Prolog BST without the web interface,
%  you can load just bst.pl:
%  ?- [bst].
